...
<pre><div class="h"></div></pre>
<textarea class="c" placeholder="CTRL+S: OPEN PAGE OUTPUT | CTRL+SHIFT+S: SAVE AS FILE" autofocus spellcheck="false" autocomplete="off" autocapitalize="false"></textarea>
<script>var wl;const lang=prompt("What language are you coding in?").trim(),h=document.querySelector(".h"),e=document.querySelector(".c"),l=document.querySelector(".l");e.addEventListener("scroll",t=>{let r=t.target.scrollTop;h.scrollTop=r,l.scrollTop=r,h.scrollLeft=t.target.scrollLeft}),l.addEventListener("scroll",t=>{let r=t.target.scrollTop;h.scrollTop=r,e.scrollTop=r});const ins=t=>{let r=e.selectionStart,a=e.selectionEnd;e.value=e.value.substring(0,r)+t+e.value.substring(a),e.selectionStart=e.selectionEnd=r+t.length-1,e.focus()};e.addEventListener("keydown",t=>{if("Tab"==t.key&&t.preventDefault(),t.ctrlKey&&t.shiftKey&&"s"==t.key||t.metaKey&&t.shiftKey&&"s"==t.key){let r=document.createElement("a");var a=prompt("file name? (with ext.)");if(!a)return;r.download=a,r.href=URL.createObjectURL(new Blob([e.value],{type:"text/plain"})),r.click()}else if(t.ctrlKey&&"s"==t.key||t.metaKey&&"s"==t.key){t.preventDefault(),wl&&wl.close();let s=window.open();s.document.write(e.value),wl=s}setTimeout(()=>{var r=t.target;switch(t.key){case"Tab":ins("    ");break;case"{":ins("}");break;case"[":ins("]");break;case"(":ins(")");break;case'"':ins('"');break;case"'":ins("'");break;case"`":ins("`")}h.innerHTML=hljs.highlight(r.value,{language:lang}).value||r.value;let a=r.scrollTop;l.scrollTop=a,h.scrollTop=a,h.scrollLeft=r.scrollLeft;let s="",n=r.value.split("\n").length;for(let c=0;c<n;c++)s+=`${c+1}<br>`;l.innerHTML=s})});</script>
```
